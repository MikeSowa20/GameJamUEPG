...
func configurar(posicao_inicial: Vector2, novo_valor: int, direcao: Vector2) -> void:
	...
	tween.tween_property(self, "position", position + direcao_inicial * 14.0, 0.22)
...
func _ao_jogador_entrar(corpo: Node2D) -> void:
	if coletado or (not corpo.is_in_group("Player") and not corpo.is_in_group("jogador")):
		return
	coletado = true
	DificuldadeGlobal.adicionar_tokens(valor)
	monitoring = false
	var tween := create_tween()
